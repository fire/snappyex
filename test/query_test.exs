...
defmodule QueryTest do
  ...
  test "encode basic types", context do
    ...
    assert [] ==
             query(
               "INSERT INTO SNAPPYEX_TEST.TEST_ENCODE (id, title, body, f, d, b, curr) VALUES (?, ?, ?, ?, ?, ?, ?)",
               [1, "Along came a spider", "This is a book", 42, 42.4242, 1234, naive_date_time]
             )

    assert [[1, "Along came a spider", "This is a book", 42.0, 42.4242, 1234, date]] =
             query("SELECT * FROM SNAPPYEX_TEST.TEST_ENCODE WHERE id = ?", [1])
  ...
  test "decode date", context do
    assert [[~N[2003-01-01 00:00:00]]] == query("VALUES DATE('2003-01-01')", [])
    assert [[~N[2023-02-03 00:00:00]]] == query("VALUES DATE('2023-02-03')", [])
    assert [[~N[2013-09-23 00:00:00]]] == query("VALUES DATE('2013-09-23')", [])
  end
  ...
  test "insert query", context do
    ...
    assert [] ==
             query("INSERT INTO SNAPPYEX_TEST.TEST_INSERT (id, text) VALUES (?, ?)", [
               43,
               "fortythree"
             ])
  ...
  test "insert prepared query", context do
    ...
    query =
      prepare(
        "Insert",
        "INSERT INTO SNAPPYEX_TEST.TEST_INSERT_PREPARED (id, text) VALUES (?, ?)",
        []
      )
