...
defmodule Snappyex.TestHelper do
  ...
  defmacro query(stat, params, opts \\ []) do
    quote do
      case Snappyex.prepare_execute(
             var!(context)[:pid],
             "",
             unquote(stat),
             unquote(params),
             unquote(defaults(opts))
           ) do
  ...
  defmacro execute(query, params, opts \\ []) do
    quote do
      case Snappyex.execute(var!(context)[:pid], unquote(query), unquote(params), unquote(opts)) do
  ...
  defmacro close(query, opts \\ []) do
    quote do
      case Snappyex.close(var!(context)[:pid], unquote(query), unquote(opts)) do
        {:ok, nil} -> :ok
        {:error, err} -> err
      end
    end
  ...
  defmacro transaction(fun, opts \\ []) do
    quote do
      Snappyex.transaction(var!(context)[:pid], unquote(fun), unquote(opts))
    end
  end
